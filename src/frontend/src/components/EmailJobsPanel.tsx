  ...
  return (
    <div className="paste-panel">
      ...
      {!loading && jobs.length === 0 && (
        <div className="jobs-empty">No LinkedIn job alert emails found.</div>
      )}
                ...
                {state === 'adding' && (
                  <div className="email-jd-form">
                    <div className="email-jd-label">
                      Paste the full job description from LinkedIn
                    </div>
                    <textarea
                      className="paste-textarea"
                      rows={5}
                      placeholder={`Go to ${job.source === 'linkedin' ? 'LinkedIn' : 'Indeed'}, open "${job.title}" at ${job.company}, and paste the full job description here…`}
